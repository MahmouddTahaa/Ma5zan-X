"""
Layer 1 — Master Data Seeder (idempotent)
"""
...
def seed_categories(conn):
    ...
    print(f"{count} rows ok ({elapsed:.0f}ms)")
...
def seed_customers(conn, count=50):
    ...
    while inserted < count:
        ...
        inserted += 1
    elapsed = (time.time() - t0) * 1000
    print(f"{inserted} rows ok ({elapsed:.0f}ms)")
...
def seed_initial_purchase_orders(conn):
    ...
    elapsed = (time.time() - t0) * 1000
    print(f"{po_count} POs, {item_count} items, {tx_count} txs ok ({elapsed:.0f}ms)")
